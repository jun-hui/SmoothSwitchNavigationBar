//
//  UINavigationController+SmoothNav.m
...
@implementation UINavigationController (SmoothNav)
...
- (void)setNeedsNavigationBackground:(CGFloat)alpha {
    ...
    // 对导航栏底部分割线做处理
    self.navigationBar.clipsToBounds = alpha == 0.0f;
}
...
//需要使用的地方：在这个方法里写返回按钮的事件处理
-(BOOL)navigationShouldPopOnBackButton
{
    return NO;//这里就是阻止 “pop” 动作
}
-(BOOL)navigationBar:(UINavigationBar *)navigationBar shouldPopItem:(UINavigationItem *)item
{
    ...
    //判断是否是手势pop
    if([vc respondsToSelector:@selector(et__updateInteractiveTransition:)]) {
        shouldPop = [self navigationShouldPopOnBackButton];
    }
    //不是手势，则执行 popViewControllerAnimated；
    if(shouldPop) {
...
-(void)pop
{
    ...
    if (topVC != nil) {
        ...
        if (coor != nil) {
            ...
            CGFloat fromAlpha = [[coor viewControllerForKey:UITransitionContextFromViewControllerKey].navBarBgAlpha floatValue];
            CGFloat toAlpha = [[coor viewControllerForKey:UITransitionContextToViewControllerKey].navBarBgAlpha floatValue];
            CGFloat nowAlpha = fromAlpha + (toAlpha - fromAlpha) * [topVC.navBarBgAlpha floatValue];
            [self setNeedsNavigationBackground:nowAlpha];
        }
    }
}
...
-(void)setTransparency:(NSString *)transparency{
    /*
    ...
    objc_setAssociatedObject(self, AlphaKey, transparency, OBJC_ASSOCIATION_COPY_NONATOMIC);
}
